...
runID <- reactive({
  runID <- unique(master.report[master.report$`Project Name` == input$`project`,]$`Run ID`)
  return(runID)
})
...
donorID <- reactive({
  donorID <- unique(master.report[master.report$`Project Name` == input$`project`,]$`Donor ID`)
  if (!input$`projectView`){
    donorID <- unique(master.report[master.report$`Run ID` %in% input$`run` &
                                      master.report$`Project Name` == input$`project`,]$`Donor ID`)
  }
  return(donorID)
})
